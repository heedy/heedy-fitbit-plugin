...
redirector = lambda x : f"{server_url}/api/fitbit/auth/{x}"
...
async def app_settings_update(request):
    ...
    await a.notify("setup", "Authorize Access", 
        description="Now that heedy has fitbit app credentials, you need to give the app access to your data.", 
        actions=[{"title": "Authorize","href": settings["authorization_uri"] + "?" +urllib.parse.urlencode({
            "response_type": "code",
...
async def auth_callback(request):
    ...
    settings = await a.settings
    response = await p.session.s.post(settings["refresh_uri"],data={
        "grant_type": "authorization_code",
        "code": code,
        "redirect_uri": redirector(appid)
    },auth=BasicAuth(settings["client_id"],settings["client_secret"]))
    resjson = await response.json()
    await a.update(meta = resjson)
    await a.notify("setup","Synchronizing...",description="Heedy is syncing your historical fitbit data. This message will disappear once all of your data is downloaded.",actions=[])
